...
class Backend {
  ...
  needVerification(context) {
    try {
      if (context.req.cookies.verifyToken) return true;
      return false;
    } catch (error) {
      return false;
    }
  ...
  async verifyAccount(context, _token) {
    if (!context.req.cookies.authToken) return null;
    const _atoken = context.req.cookies.authToken;
    if (!_atoken) return null;

    const user = JwonWebToken.verify(_atoken, JWT_SECRET);

    if (`${user.verificationCode}` == `${_token}`) {
      context.res.setHeader(
        "Set-Cookie",
        cookie.serialize("authToken", null, {
          httpOnly: true,
    ...
    return { success: false, newToken: _atoken };
  }
